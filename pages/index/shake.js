...
Page({
  ...
  fetch(){
    let that=this
    let s=wx.getStorageSync('state')
    wx.request({
      url: app.globalData.config.apiUrl + 'tmall.php?act=getPartyStatus',
      data: {
      },
      method: 'POST',
      success: (res) => {
       ...
       if(data==1){
         if(s==0){
           ...
           that.setData({
             ...
             flag:1,
             hasResult:1,
             txt:wx.getStorageSync('userText')
           })
         }
  ...
  onAccelerometerChange(res) {
    let that = this
    let s=wx.getStorageSync('state')
      if (res.x > .7 && res.y > .7 && that.data.flag < 1&&s==0) {
